...
class AllianceDecisionNode(Node):
    ...
    def __init__(self):
        ...
        self.supply_need_hp = False          # 本轮补给是否等待回血完成
        self.supply_need_ammo = False        # 本轮补给是否等待补弹完成
        self.force_supply_after_respawn = False  # 复活后是否必须先补给
    ...
    def _handle_supply(self):
        ...
        if not self.supply_goal_sent:
            ...
            self._log_info(
                f"💊 前往补给点 ({SUPPLY_POINT['x']}, {SUPPLY_POINT['y']})"
                f" | 原因: {reason}"
            )
        ...
        if self._is_supply_complete():
            ...
            self._log_info(
                f"✅ 补给完成 HP={hp}/{max_hp} | Ammo={ammo_text}, 恢复巡逻!"
            )
            self._exit_supply()
            return
        ...
        if not hasattr(self, '_last_supply_log') or \
            ...
            self._log_info(
                ...
                f"| Ammo={ammo_text} | 已执行补给任务 {time_in_supply:.1f}s"
            )
    ...
    def _print_banner(self):
        lines = [
            "",
            box_top(),
            box_line("ADAM 2026UC 哨兵决策系统 v2"),
            box_line(""),
