...
class RegionMonitorNode(Node):
    ...
    def _bump_override_callback(self):
        ...
        # 持续刷新 region 编码，确保串口节点稳定收到颠簸区域状态。
        region_msg = UInt8()
        region_msg.data = BUMP_REGION_CODE
        self.region_pub.publish(region_msg)
    ...
    def _update_big_yaw_aligned(self):
        ...
        if aligned != self.big_yaw_aligned:
            self.get_logger().info(
                f'[BUMP] 大yaw对齐标志切换: {int(self.big_yaw_aligned)} -> {int(aligned)}')
        self.big_yaw_aligned = aligned
        self._publish_big_yaw_aligned()

    def _publish_big_yaw_aligned(self):
        msg = UInt8()
        msg.data = 1 if self.big_yaw_aligned else 0
        self.big_yaw_aligned_pub.publish(msg)
    ...
    def _publish_selected_yaw(self):
        ...
        if now - self.last_yaw_log_time >= 0.5:
            ...
            self.get_logger().info(
                ...
                f'对齐误差={yaw_delta_str}度，'
                f'下发角度={sent_yaw_str}度')
